...
All data is synthetic and is not medical advice. Directly under `fabric/` are small CSV files corresponding to all four Ontology entity types, allowing relationships to be verified using the same `*-SYN-*` IDs. `fabric/enterprise/` contains large CSV files that can be reproduced from the generator.

`work-iq/` contains four items that refer to the same synthetic patient and encounter: a SharePoint document, a Teams conversation, an Outlook email, and meeting notes. Use them only with dedicated test accounts, and do not enter real patient information.
