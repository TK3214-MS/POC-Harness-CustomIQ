...
1. Fabric portalでラボ用workspaceを開き、対象capacityへ割り当てられていることを確認する。
2. **New item > Lakehouse**を選択し、`IIQ<Industry>LH`を作成する。
3. Lakehouse explorerの**Files**で`demo` folderを作成する。
4. **Get data > Upload files**から[`industry-packs/<pack>/sample-data/fabric/*.csv`](https://github.com/TK3214-MS/POC-Harness-CustomIQ/tree/main/industry-packs){ target="_blank" rel="noopener" }を`Files/demo/`へアップロードする。
5. CSVごとに**Load to tables > New table**を選び、拡張子を除いた名前のmanaged tableへ変換する。
6. table previewでID列をstring、日時列をdatetime、数値列を適切な数値型として確認する。
7. 各tableのrow countを[デモデータ投入runbook](../../evaluation/Demo-Data-Deployment-Runbook.md)の小容量datasetと照合する。
...
1. [`industry-packs/<pack>/ontology/entities.yaml`](https://github.com/TK3214-MS/POC-Harness-CustomIQ/tree/main/industry-packs){ target="_blank" rel="noopener" }を開き、`entity_types`を確認する。
2. workspaceで**New item > Ontology (preview)**を選び、`IIQ<Industry>Ontology`を作成する。
3. **Build directly from OneLake**を選択する。
4. Home canvasの**Add entity type**から`entity_types`の各`name`を作成する。
5. entity typeの**Bind data > Add data binding > Lakehouse table**から`dataset_key`と同名のmanaged tableを選択する。
6. **Configure > Manage property bindings > Add properties**で定義済みpropertyをsource列へ対応付ける。
7. **Define entity type key**で`identifier_field`を選択し、display name propertyを設定して保存する。
8. **Instances**を開き、CSV内の既知IDを検索してproperty値を照合する。
