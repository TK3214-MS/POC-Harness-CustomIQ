...
すべて合成データである。`fabric/`直下にはOntologyの全4 entity typeに対応する小容量CSVがあり、同じ`*-SYN-*` IDでrelationshipを検証できる。`fabric/enterprise/`にはgeneratorから再現可能な大規模CSVがある。

`work-iq/`には同一不正ケースを参照するSharePoint文書、Teams会話、Outlookメール、会議記録の4件がある。専用テストアカウントへ登録し、実在する顧客・口座情報と混在させない。
