...
All data is synthetic. Directly under `fabric/` are small CSV files corresponding to all five Ontology entity types, allowing relationships to be verified using the same `*-SYN-*` IDs. `fabric/enterprise/` contains large CSV files that can be reproduced from the generator.

`work-iq/` contains four items that refer to the same demand and inventory case: a SharePoint document, a Teams conversation, an Outlook email, and meeting notes. Register them in dedicated test accounts.
