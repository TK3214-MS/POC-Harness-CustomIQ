...
- **ACR の最小権限構成**: Azure Container Registry は admin ユーザーを無効化（`adminUserEnabled: false`）し、User-Assigned Managed Identity に組み込みロール `AcrPull` のみを付与しています（[deployment/bicep/resources.bicep](deployment/bicep/resources.bicep)、[ADR-0012](docs/decisions/0012-mcp-backend-deployment-target.md)）。
...
上記の詳細と、その他の未実装項目全体の一覧は [docs/Known-Limitations.md](docs/Known-Limitations.md) を参照してください。
