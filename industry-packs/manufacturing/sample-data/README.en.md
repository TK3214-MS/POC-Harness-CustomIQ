...
All data is synthetic. Directly under `fabric/` are small CSV files corresponding to all six Ontology entity types, allowing relationships to be verified using the same `*-SYN-*` IDs. `fabric/enterprise/` contains large CSV files that can be reproduced from the generator.

`work-iq/` contains four items that refer to the same quality case: a SharePoint document, a Teams conversation, an Outlook email, and meeting notes. Manually register them in the corresponding M365 areas of dedicated test accounts.

The eight documents in `knowledge/` are managed documents to register with the Foundry IQ Blob Knowledge Source. `foundry/demo/knowledge.jsonl` is a supplemental corpus that bundles the eight documents, and `foundry/enterprise/records.jsonl` is for validating searches over structured business records; neither replaces the managed Markdown documents. `prompts/enterprise_prompts.yaml` contains 50 evaluation questions across five scenarios.
